...
- **Railway config-as-code is deprecated and ends 2026-12-01.** Railway stops reading `railway.toml` then, and since 2026-08-28 a service that never used a config file cannot opt in. So the two services are configured differently, and neither will change by editing a toml:
  - **Web (`tardbox`):** Dockerfile-based deploy at `https://tardbox.com`. `railway.toml` supplies builder, Dockerfile path, watch patterns and `healthcheckPath = "/health"`, and while it does, the dashboard shows those fields locked as "set in railway.toml". **Before 2026-12-01** delete `railway.toml` (which unlocks them) and set the same values in the dashboard; each step redeploys, and each redeploy is ~2 minutes of 502s because of the volume. Railway's `PORT` env var is mapped to `BB_PORT` in the Dockerfile `CMD`. Domain via Namecheap DNS CNAME. Storage is `BB_STORAGE=FS` on an attached Railway volume, not S3.
...
- **Watch paths limit redeploys to changes that reach the image** (`src/`, `resources/`, `deps.edn`, `build.clj`, `Dockerfile`; the web's list also has `railway.toml` while that exists). A docs-only push used to rebuild twice and redeploy both services, and each web redeploy is an outage. A new file the image depends on must be added to **both** services' lists — one in `railway.toml`, one in the bot's dashboard.
