...
- `src/boostbox/feed.clj` — Reads three things out of an RSS document: the npubs the feed declares, a fediverse account, and a cover image. Pure
...
- `src/boostbox/mastodon.clj` — The second destination a boost is announced on. Bearer token, two POSTs, and nothing that throws
...
- `scripts/mastodon-check.sh` — check a Mastodon token before setting it on the deployed bot: whose account it is, whether the instance considers it a bot, the instance's real character limit, and whether the banner renders in-process. Posts nothing
...
**Boost Bot:** A second process — `java -cp boostbox.jar boostbox.nostrbot` — that watches a Lightning wallet over NWC, stores incoming boostagrams via `POST /boost`, and announces them: to Nostr as `kind:1` notes with NIP-73 `i`/`k` tags, and to Mastodon when one is configured. It is deliberately NOT a route on the web app: it holds an nsec and a wallet credential, and it is a long-lived loop with at-least-once delivery. `build.clj` compiles everything under `src/`, so its `(:gen-class)` lands in the same uberjar automatically — no build changes needed to add an entry point.
...
**Mastodon is a destination, not a second product (`boostbox.mastodon`, `nostrbot/publish-mastodon!`).** The 1% split buys a podcaster the announcement; which networks it lands on is ours to decide, so this is another target inside the same poll loop rather than a sibling process — one cursor, one dedupe entry, one feed read shared by both. It is off entirely without `BBN_MASTODON_URL` *and* `BBN_MASTODON_TOKEN`, exactly as `podcastindex` is off without its credentials, and nothing in the namespace throws. **The two destinations fail differently on purpose:** a relay rejection throws and holds the cursor, because Nostr is the announcement of record and a retry is free; a Mastodon failure is logged, recorded as `"mastodon" "failed"` in the state entry, and passed over, because holding the cursor to retry a post would stop every *later* boost being announced anywhere. At-least-once there, at-most-once here. A retry would still be safe: `post-status!` sends the payment hash as an `Idempotency-Key`, which Mastodon deduplicates on, so a crash between posting and saving state cannot double-post.

**Two Mastodon API traps, both of which fail as a good-looking post.** `POST /api/v2/media` answers `200` when an attachment is ready and **`202` when it is still processing** — both carry an id, and attaching a 202's id yields a post with a blank image, which looks like success at every point anyone would check; `upload-media!` polls `/api/v1/media/:id` until ready and drops the id past a deadline, so the boost posts with no picture rather than an empty frame. And the banner is drawn **in the bot process** with `banner/banner-png` rather than fetched from the web app's own `/og/boost.png`, from the same `boostagram/banner-params` the note's URL is built from — which is why `nostrbot/-main` must set `java.awt.headless`. It is set in `boostbox/-main` and the Dockerfile `CMD`, and `railway.bot.toml`'s `startCommand` is a bare `java -cp`, so the bot inherits neither; without it the first draw throws, and a missing font draws blank.

**Payer-written text cannot address anyone or go anywhere (`boostagram/defang`).** Mastodon *parses the body of a status*: `@user@host` becomes a real mention that lands in that account's notifications, and `#word` files the post under that hashtag. Both would then be chosen by whoever paid, on a post the bot published under its own name — the same attack the `sender`-not-`p` rule above stops on Nostr, and on this network also the fastest route to a suspension. A zero-width space after the sigil stops the parser matching while leaving the text reading exactly as written; nothing is deleted. Applied to the message, the show, the episode and the sender name — every payer-written field — and never to the post's own two hashtags or the app label, which are ours.

**The fediverse account is linked, never mentioned, and only ever comes from the feed (`feed/fediverse-profile`).** This is the Mastodon counterpart of a note's `p` tags and inherits their rule: the value comes from a podcaster naming their own account in their own RSS, never from the payment. It is emitted as a plain URL so it credits and never notifies. **The returned string is built, not echoed** — a feed is reached by a URL whoever paid us chose, so its contents are only as trustworthy as that payer, and passing one out verbatim would put an arbitrary link under the bot's identity, which is the rule directly below. Instead the host and username are each shape-checked and a fresh `https://host/@user` is assembled from the two, which is also what normalizes the three spellings feeds use in the wild. An attacker can still name their own host; they cannot choose the scheme, the path, or anything else that gets printed.

**`feed/txt-blocks` exists because `find-tags` is eager.** It walks and allocates an entry for *every* open tag before anything downstream takes its first few, so two readers each calling `find-blocks xml "podcast:txt"` doubles the cost of exactly the document `the-scan-is-linear` pins — the second caller is what pushes it over the limit, and it measured 8.5 s there. `read-feed` finds them once and hands the same lazy seq to `feed-npubs` and `feed-fediverse`. Any third reader of that tag takes it as an argument too.

**The note's body is BMB's; the post's body is the note's, minus what Mastodon cannot use (`boostagram/->mastodon-content`).** Same layout, same `note-sats`, so a reader following the bot in both places sees one kind of card and a post and a note never disagree about one payment. Four deliberate differences: no banner URL (it is a real attachment, so the link would be a duplicate paid for out of the character budget); no npub anywhere (a Mastodon client cannot resolve one); the fediverse link above; and a hard length cap in which **only the payer's message gives way** — the attribution, the app link and the credited account are what the post is for. The budget is counted conservatively: Java counts an astral character as two where Mastodon counts one, and Mastodon counts any URL as 23 however long it is, so the post is always measured as at least as long as the server will measure it. The BoostBox permalink is left out for the same reason `->note-content` leaves it out.
