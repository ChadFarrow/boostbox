...
**A transaction that publishes nothing says why (`nwc/transaction->boost`).** It answers either a boost or `{:skip <reason>}`, never nil, and `poll-once!` logs the reason per transaction plus a `:skipped` breakdown on the poll line. The four reasons are not interchangeable: `:no-boostagram` is an ordinary payment and `:not-a-boost` is a stream — both expected, and the first is only counted because a wallet taking normal payments sees it constantly — while `:tlv-undecodable` and `:tlv-unparseable` are defects. This exists because all four used to be the same nil against a poll line that only counted, so a boost that went missing left no trace anywhere and diagnosing one meant asking the payer to send their TLV by hand. `tx->boost!` preserves the reason through the boost-link fallback and attaches the payment hash, so a missing boost is searchable by the id the wallet shows. Separately, `::no-cursor-window-dropped` fires whenever the bot starts with no cursor and no backfill — the silent-drop case that a lost state file causes.
